...
class WorkoutManager: ObservableObject {
    ...
    private func setupSubscriptions() {
        locationService.$totalDistance
            .sink { [weak self] distance in
                guard let self = self, var session = self.session else { return }
                session.state.totalDistance = distance
                session.state.intervalDistance = distance - self.intervalStartDistance
        ...
        locationService.$currentPace
            .sink { [weak self] pace in
                guard let self = self, var session = self.session else { return }
                session.state.currentPace = pace
                self.session = session
            }
            .store(in: &cancellables)
    }
    ...
    func startWorkout(_ workout: Workout) {
        ...
        guard !isActive else { 
            print("ERROR: Workout already active")
            return 
        }
        ...
        do {
            var newSession = WorkoutSession(workout: workout)
            newSession.startTime = Date()
            newSession.state.isActive = true
            newSession.state.currentIntervalType = .warmUp
            newSession.state.intervalStartTime = Date()
            newSession.state.remainingIntervals = workout.numberOfIntervals
            ...
            print("✗ FATAL ERROR creating session: \(error)")
            isActive = false
            session = nil
            return
        }
